...
import {
  ...
  searchPublishUsers, setUserPublishPermission, updateProjectCopyLabel, rebuildProjectSnapshot,
...
export default function PublishManage() {
  ...
  const snapshotRebuildDialogRef = useRef<number | null>(null);
  const [snapshotRebuildDialogProjectId, setSnapshotRebuildDialogProjectId] = useState<number | null>(null);
  const [snapshotRebuildPendingProjectId, setSnapshotRebuildPendingProjectId] = useState<number | null>(null);

  const openCopyLabel = (row: PublishItem) => {
    let copyLabel = row.copyLabel || '复制';
    Modal.confirm({
      title: `修改复制标签：${row.projectName}`,
      width: 460,
      content: (
        <div>
          <div className="help-text publish-action-tip">标签会显示在复制来源项目上；留空将恢复默认“复制”。</div>
          <Input
            defaultValue={copyLabel}
            maxLength={32}
            showCount
            placeholder="复制"
            onChange={(event) => { copyLabel = event.target.value; }}
          />
        </div>
      ),
      okText: '保存',
      cancelText: '取消',
      onOk: () => updateProjectCopyLabel({ id: row.id, copyLabel: copyLabel.trim() || null })
        .then(() => {
          message.success('复制标签修改成功');
          publish.reload();
        })
    });
  };

  const openSnapshotRebuild = (row: PublishItem) => {
    if (snapshotRebuildDialogRef.current !== null) return;
    snapshotRebuildDialogRef.current = row.id;
    setSnapshotRebuildDialogProjectId(row.id);
    let reason = '';
    let submitPromise: Promise<void> | null = null;
    let modal: ReturnType<typeof Modal.confirm>;
    modal = Modal.confirm({
      title: `重建发布快照：${row.projectName}`,
      width: 500,
      content: (
        <div>
          <div className="help-text publish-action-tip">
            用于补齐历史快照缺失字段；系统会核对已审核内容未变化，相同内容不会重复生成 revision。
          </div>
          <Input.TextArea
            rows={3}
            maxLength={200}
            showCount
            placeholder="请输入重建原因（必填）"
      ...
      maskClosable: false,
      afterClose: () => {
        snapshotRebuildDialogRef.current = null;
        setSnapshotRebuildDialogProjectId(null);
      },
      onOk: () => {
        if (!reason.trim()) {
        ...
        setSnapshotRebuildPendingProjectId(row.id);
        modal.update({
        ...
        submitPromise = (async () => {
          await rebuildProjectSnapshot({ id: row.id, reason: reason.trim() });
          message.success('发布快照重建成功');
          publish.reload();
        })().catch((error) => {
          submitPromise = null;
          modal.update({
          ...
          setSnapshotRebuildPendingProjectId(null);
        });
  ...
  const publishColumns: ColumnsType<PublishItem> = [
    ...
    {
      title: '流程权限', key: 'flowPermission', width: 150,
      render: (_: any, row) => (
        <Space size={4}>
          <Tag color={row.allowPreview === '1' ? 'blue' : 'default'}>预览</Tag>
          <Tag color={row.allowCopy === '1' ? 'green' : 'default'}>复制</Tag>
        </Space>
      )
    },
    {
      title: '复制标签', dataIndex: 'copyLabel', width: 110,
      render: (value: string, row) => row.sourceProjectId
        ? <Tag color="orange">{value || '复制'}</Tag>
        : <span className="help-text">原创</span>
    },
    { title: '原因', dataIndex: 'statusReason', ellipsis: true },
    { title: '发布时间', dataIndex: 'publishTime', width: 165 },
    {
      title: '操作', key: 'op', width: 360, fixed: 'right',
      render: (_: any, row) => (
        <Space size={4}>
          ...
          {canEdit && row.status === 4 && row.isPublic === '1' && (
            <Button
              type="link"
              size="small"
              disabled={snapshotRebuildDialogProjectId !== null}
              loading={snapshotRebuildPendingProjectId === row.id}
              onClick={() => openSnapshotRebuild(row)}
            >
              重建快照
            </Button>
          )}
          {canEdit && row.sourceProjectId && (
            <Button type="link" size="small" onClick={() => openCopyLabel(row)}>修改标签</Button>
          )}
        </Space>
      )
    }

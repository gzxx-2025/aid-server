...
const copyLabelInflight = new Map<string, Promise<unknown>>();
const snapshotRebuildInflight = new Map<number, Promise<unknown>>();

/** 修改复制项目标签；相同业务键的进行中请求只提交一次。 */
export function updateProjectCopyLabel(data: { id: number; copyLabel?: string | null }) {
  const key = `${data.id}:${(data.copyLabel || '').trim()}`;
  const current = copyLabelInflight.get(key);
  if (current) return current;
  const task = request({ url: '/aid/project-copy/label', method: 'post', data })
    .finally(() => copyLabelInflight.delete(key));
  copyLabelInflight.set(key, task);
  return task;
}

/** 重建已发布项目的不可变发布快照；同一项目的进行中请求只提交一次。 */
export function rebuildProjectSnapshot(data: { id: number; reason: string }) {
  const current = snapshotRebuildInflight.get(data.id);
  if (current) return current;
  const task = request({ url: '/aid/project-copy/snapshot/rebuild', method: 'post', data })
    .finally(() => snapshotRebuildInflight.delete(data.id));
  snapshotRebuildInflight.set(data.id, task);
  return task;
}

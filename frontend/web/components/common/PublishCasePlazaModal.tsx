...
export function PublishCasePlazaModal({
  ...
  const [isMovieProject, setIsMovieProject] = useState(false)
  ...
  async function hydrateForm() {
    ...
    try {
      ...
      const detailAllowPreview = detail.allowPreview === true
      const detailIsMovie = detail.projectType === 'movie'
      setIsMovieProject(detailIsMovie)
      setAllowPreview(detailAllowPreview)
      setAllowCopy(detailIsMovie && detailAllowPreview && detail.allowCopy === true)
    } catch (e: unknown) {
      if (hydrateSeq !== hydrateSeqRef.current) return
      const err = e as { msg?: string; message?: string }
      // 发布权限必须以详情接口为准，加载失败时不允许带默认值覆盖已有设置。
      message.error(err?.msg || err?.message || '加载项目信息失败')
      onOpenChange(false)
    } finally {
      if (hydrateSeq === hydrateSeqRef.current) setHydrating(false)
    }
  }
  ...
  async function handleConfirm(e?: { preventDefault?: () => void }) {
    ...
    try {
      await userProjectUpdate({
        ...
        allowPreview,
        allowCopy: isMovieProject && allowPreview && allowCopy
      })
    ...
    >
      {hydrating ? (
        ...
        <div className="publish-case-plaza">
          ...
          <div className="publish-case-plaza__permission-panel">
            <div className="publish-case-plaza__permission-row">
              <div>
                <div className="publish-case-plaza__permission-title">允许预览项目流程</div>
                <div className="publish-case-plaza__permission-desc">访客可读取审核通过的流程快照；之后编辑草稿不会改变该版本。</div>
              </div>
              <Switch
                checked={allowPreview}
                disabled={busy}
                onChange={(checked) => {
                  setAllowPreview(checked)
                  if (!checked) setAllowCopy(false)
                }}
              />
            </div>
            <div className="publish-case-plaza__permission-row">
              <div>
                <div className="publish-case-plaza__permission-title">允许复制电影工程</div>
                <div className="publish-case-plaza__permission-desc">
                  {isMovieProject
                    ? '只复制到预览时间线，不包含最终成片；复制者需自行合成导出。'
                    : '剧集项目不支持复制。'}
                </div>
              </div>
              <Switch
                checked={allowCopy}
                disabled={busy || !allowPreview || !isMovieProject}

...
public class AidUserProfileServiceImpl extends ServiceImpl<AidUserProfileMapper, AidUserProfile> implements IAidUserProfileService
{
    ...
    /**
     * 按 userId 精简查询用户账户积分信息。
     */
    @Override
    public AidUserProfile getAccountBalanceByUserId(Long userId)
    {
        if (Objects.isNull(userId))
        {
            return null;
        }
        // 余额快捷接口只需要账户相关字段，后续新增字段时需同步确认是否需要返回
        LambdaQueryWrapper<AidUserProfile> wrapper = Wrappers.lambdaQuery();
        wrapper.select(AidUserProfile::getBalance,
                        AidUserProfile::getFrozenBalance,
                        AidUserProfile::getTotalRecharge,
                        AidUserProfile::getTotalConsumption)

...
public class AidMediaTask extends BaseEntity {
    ...
    /**
     * 编排父任务ID（aid_extract_task.id）。
     * 与 biz_task_id 的业务记录定位职责分离：本字段只用于父子任务续租、重启对账与终态汇总。
     */
    ...
    /**
     * 上游受理时刻：拿到 provider_task_id、接入调度中心的那一刻。
     * 最大存活从此刻起算而非 create_time，把「并发排队等待」排除在出片耗时之外。
     */
    @TableField("upstream_accept_time")
    private java.util.Date upstreamAcceptTime;

    /**
     * 最近一次观测到上游仍在推进的时刻：轮询拿到「处理中」、或回调带回非终态时前移。
     * 无进展超时以此为起算点，是判定上游生死的唯一依据。
     */

...
public class JimengImageProviderClient implements ImageProviderClient {
    ...
    /** 1K 档位 —— 4.0/4.6 满足官方面积、比例范围的标准宽高 */
    ...
    static {
        ...
        // --- 1K（4.0/4.6：官方只列方图推荐值，其余按面积下限与目标比例生成 16 倍数尺寸） ---
        Map<String, int[]> m1k = new HashMap<>();
        m1k.put("1:1", new int[]{1024, 1024});
        m1k.put("4:3", new int[]{1184, 896});
        m1k.put("3:4", new int[]{896, 1184});
        m1k.put("3:2", new int[]{1264, 832});
        m1k.put("2:3", new int[]{832, 1264});
        m1k.put("16:9", new int[]{1376, 768});
        m1k.put("9:16", new int[]{768, 1376});
        m1k.put("21:9", new int[]{1568, 672});
        m1k.put("9:21", new int[]{672, 1568});
    ...
    public ProviderSubmitResult submit(AiModelConfigVo modelConfig, MediaImageGenerateRequest request) {
        ...
        for (int attempt = 1; ; attempt++) {
            ...
            if (isTransientSubmitFailure(code, raw) && attempt < JIMENG_SUBMIT_MAX_ATTEMPTS) {
                log.warn("即梦图片提交瞬时失败将重试, modelCode={}, code={}, attempt={}/{}, responseLen={}",
                        modelCode, code, attempt, JIMENG_SUBMIT_MAX_ATTEMPTS, StringUtils.length(raw));
                sleepBackoff(attempt);
                continue;
            }
        ...
        if (StrUtil.isBlank(taskId)) {
            log.error("即梦图片提交成功但未返回 task_id, modelCode={}, responseLen={}",
                    modelCode, StringUtils.length(raw));
            return ProviderSubmitResult.builder()
                    .rawResponse(raw)
                    .build();
        }
    ...
    public ProviderTaskResult query(AiModelConfigVo modelConfig, String providerTaskId) {
        ...
        try {
            ...
            return queryAnomaly(null, StrUtil.blankToDefault(e.getMessage(), "即梦查询失败"), null);
        ...
        if (code != JimengConstants.RESP_CODE_SUCCESS) {
            ...
            if (isDocumentedTerminalStatus(vendorStatus)) {
                return ProviderTaskResult.builder()
                        .status(JimengConstants.TASK_STATUS_FAILED)
                        .errorMessage(StrUtil.blankToDefault(message, "即梦任务失败"))
                        .rawResponse(raw)
                        .querySuccessful(Boolean.TRUE)
                        .providerStatus(vendorStatus)
                        .terminalConfirmed(Boolean.TRUE)
                        .build();
            }
            return queryAnomaly(raw, StrUtil.blankToDefault(message, "即梦查询失败"), vendorStatus);
        }

        if (!isKnownStatus(vendorStatus)) {
            return queryAnomaly(raw, StrUtil.isBlank(vendorStatus)
                    ? "上游响应缺少状态" : "上游返回未知状态:" + vendorStatus, vendorStatus);
        }
        ...
        if (JimengConstants.TASK_STATUS_SUCCEEDED.equals(normalized)) {
            ...
            if (StrUtil.isBlank(resultUrl)) {
                log.error("即梦图片 status=done 但未解析到结果图 URL, modelCode={}, taskId={}, responseLen={}",
                        modelCode, providerTaskId, StringUtils.length(raw));
                return queryAnomaly(raw, "上游成功但结果链接未就绪", vendorStatus);
            }
        ...
        return ProviderTaskResult.builder()
                ...
                .querySuccessful(Boolean.TRUE)
                .providerStatus(vendorStatus)
                .terminalConfirmed(isDocumentedTerminalStatus(vendorStatus))
                .build();
    }
    ...
    private String doSignedPost(AiModelConfigVo modelConfig, String action, Map<String, Object> body) {
        ...
        try (HttpResponse response = post.execute()) {
            ...
            if (!response.isOk()) {
                log.error("即梦 HTTP 非 2xx, status={}, responseLen={}",
                        response.getStatus(), StringUtils.length(raw));
            }
            return raw;
        }
    }
    ...
    private boolean isKnownStatus(String vendorStatus) {
        if (StrUtil.isBlank(vendorStatus)) {
            return false;
        }
        String lower = vendorStatus.trim().toLowerCase(Locale.ROOT);
        return JimengConstants.VENDOR_STATUS_IN_QUEUE.equals(lower)
                || JimengConstants.VENDOR_STATUS_GENERATING.equals(lower)
                || JimengConstants.VENDOR_STATUS_DONE.equals(lower)
                || JimengConstants.VENDOR_STATUS_NOT_FOUND.equals(lower)
                || JimengConstants.VENDOR_STATUS_EXPIRED.equals(lower);
    }

    private boolean isDocumentedTerminalStatus(String vendorStatus) {
        if (StrUtil.isBlank(vendorStatus)) {
            return false;
        }
        String lower = vendorStatus.trim().toLowerCase(Locale.ROOT);
        return JimengConstants.VENDOR_STATUS_DONE.equals(lower)
                || JimengConstants.VENDOR_STATUS_NOT_FOUND.equals(lower)
                || JimengConstants.VENDOR_STATUS_EXPIRED.equals(lower);
    }
    ...
    private ProviderTaskResult queryAnomaly(String raw, String message, String providerStatus) {
        return ProviderTaskResult.builder()
                ...
                .errorMessage(StrUtil.blankToDefault(message, "上游状态待确认"))
                .rawResponse(raw)

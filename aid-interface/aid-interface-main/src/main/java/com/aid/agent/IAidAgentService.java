...
public interface IAidAgentService extends IService<AidAgent>
{
    ...
    /**
     * 根据 agent_code 查询提示词缓存校验所需的最小字段。
     * 只 select id / status / update_time / create_time，不含 prompt_content，新增校验字段时需同步。
     *
     * @param agentCode 智能体编码
     * @return 仅含校验字段的实体；不存在返回 null
     */
    AidAgent getPromptMetaByAgentCode(String agentCode);

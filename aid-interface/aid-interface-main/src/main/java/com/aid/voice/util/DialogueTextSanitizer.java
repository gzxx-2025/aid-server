...
import com.aid.common.text.RichTextNormalizer;
 ...
 *
 * <p>需要清洗的标记形态（来自分镜脚本 / 视觉导演产出 / 富文本编辑器）：</p>
 * <ul>
 *   <li>HTML 标签与实体 {@code <p>…</p>} / {@code &nbsp;} —— 富文本编辑器写入的排版标记；</li>
...
public final class DialogueTextSanitizer
{
    ...
    public static boolean isNoDialoguePlaceholder(String text) {
        ...
        // 富文本入参（<p>无</p>）先归一为纯文本，否则占位判定会被排版标签挡住
        String plainText = RichTextNormalizer.toPlainText(text);
        return StrUtil.isNotBlank(plainText) && NO_DIALOGUE_PLACEHOLDER.matcher(plainText).matches();
    }

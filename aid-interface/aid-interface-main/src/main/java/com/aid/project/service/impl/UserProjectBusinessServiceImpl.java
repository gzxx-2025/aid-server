...
public class UserProjectBusinessServiceImpl implements IUserProjectBusinessService
{
    ...
    public List<AidComicProject> selectUserProjectList(UserProjectQueryRequest request, Long userId)
    {
        ...
        wrapper.select(AidComicProject::getId, AidComicProject::getUserId,
                ...
                AidComicProject::getAllowPreview, AidComicProject::getAllowCopy,
                AidComicProject::getPublishedSnapshotId, AidComicProject::getSourceProjectId,
                AidComicProject::getSourceSnapshotId, AidComicProject::getCopyLabel,
                AidComicProject::getCopyCount,
        ...
        // 兼容尚未接入分页的现有客户端：仅在请求体明确携带分页参数时开启分页。
        // 分页紧邻列表查询开启，pageSize 由统一工具钳制，防止超大分页拉爆内存。
        if (Objects.nonNull(request.getPageNum()) || Objects.nonNull(request.getPageSize())) {
            SafePageUtils.startClampedPage(request.getPageNum(), request.getPageSize());
        }
    ...
    public AidComicProject insertUserProject(UserProjectCreateRequest request, Long userId)
    {
        ...
        project.setAllowPreview(IS_PUBLIC_NO);
        project.setAllowCopy(IS_PUBLIC_NO);
        project.setCopyCount(0L);
        // 未删除
    ...
    public AidComicProject updateUserProject(UserProjectUpdateRequest request, Long userId)
    {
        ...
        // 发布流程权限属于可随时调整的展示开关，不改变已审核快照内容。
        if (Boolean.FALSE.equals(request.getAllowPreview()) && Boolean.TRUE.equals(request.getAllowCopy())) {
            log.info("修改项目复制权限拒绝，请求开关矛盾: projectId={}", project.getId());
            throw new ServiceException("请先允许预览");
        }
        if (Boolean.FALSE.equals(request.getAllowPreview())) {
            project.setAllowPreview(IS_PUBLIC_NO);
            // 复制必然会读取快照；关闭预览时同步关闭复制，避免形成矛盾权限。
            project.setAllowCopy(IS_PUBLIC_NO);
        } else {
            if (Boolean.TRUE.equals(request.getAllowPreview())) {
                project.setAllowPreview(IS_PUBLIC_YES);
            }
            if (request.getAllowCopy() != null) {
                if (Boolean.TRUE.equals(request.getAllowCopy())
                        && !Objects.equals(ProjectTypeEnum.MOVIE.getValue(), project.getProjectType())) {
                    log.info("修改项目复制权限拒绝，类型不支持: projectId={}, type={}",
                            project.getId(), project.getProjectType());
                    throw new ServiceException("仅支持电影复制");
                }
                boolean previewEnabled = Objects.equals(IS_PUBLIC_YES, project.getAllowPreview());
                if (Boolean.TRUE.equals(request.getAllowCopy()) && !previewEnabled) {
                    log.info("修改项目复制权限拒绝，预览未开启: projectId={}", project.getId());
                    throw new ServiceException("请先允许预览");
                }
                project.setAllowCopy(Boolean.TRUE.equals(request.getAllowCopy())
                        ? IS_PUBLIC_YES : IS_PUBLIC_NO);
            }
        }
        if (!changingStyle) {
    ...
    {
        // 与复制、权限切换和审核共用项目行锁，避免删除期间又派生出新的复制工程。
        AidComicProject project = this.selectUserProjectByIdForUpdate(id, userId);
        if (project == null) {
            throw new RuntimeException("项目不存在或无权限操作");
        }
    ...
    public AidComicProject submitAudit(Long id, Long userId)
    {
        ...
        if (Objects.equals(beforeStatus, ProjectStatusEnum.AUDIT_PASSED.getValue())
                ...
                && Objects.nonNull(project.getPublishedSnapshotId())) {
                                         ...
                                         Long episodeCount, boolean styleLocked)
    {
        UserProjectVO.UserProjectVOBuilder builder = UserProjectVO.builder()
                ...
                .allowCopy(Objects.equals(IS_PUBLIC_YES, project.getAllowCopy()))
                ...
                .copyCount(Objects.requireNonNullElse(project.getCopyCount(), 0L))

 ...
 * 按项目 + 剧集 + 类型（图片 / 分镜视频 / 配音视频）过滤当前用户的生成记录，可选按分镜进一步筛选。
 * {@code episodeId} 取值：电影项目传 {@code 0}，剧集项目传 &gt; 0 的实际剧集 ID。
 * 是否被选为最终分镜由返回 VO 的 {@code isSelected} 字段表示。
 *
 * @author 视觉AID
 */
@Data

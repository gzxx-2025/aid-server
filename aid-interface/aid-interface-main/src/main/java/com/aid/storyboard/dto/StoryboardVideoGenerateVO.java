...
/**
 * 分镜图生视频响应 VO（支持单/多镜头批量）。
 *
 * @author 视觉AID
 */
@Data
public class StoryboardVideoGenerateVO
{
    ...
    /** 每个镜头的出片条数（多镜头恒为 1；单镜头为 count，1~4） */
    private Integer countPerShot;

    /** 子任务总数 = totalShots × countPerShot（= 本次提交的视频总条数） */
    ...
    public static class ShotResult
    {
        ...
        /** 是否已受理进入父任务（true=已纳入出片；false=被跳过，见 reason） */
        private Boolean accepted;

        /** 未受理原因（accepted=false 时有值，如「任务处理中」「请选参考图」「请先生成提示词」等） */

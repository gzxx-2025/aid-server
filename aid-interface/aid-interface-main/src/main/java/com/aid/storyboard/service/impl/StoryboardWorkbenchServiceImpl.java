...
public class StoryboardWorkbenchServiceImpl implements IStoryboardWorkbenchService {
    ...
    /** 正式配音支持的音频格式（Provider 能力并集，统一任务入口会再次兜底校验）。 */
    private static final Set<String> TTS_AUDIO_FORMATS = Set.of("mp3", "wav", "pcm", "ogg_opus", "opus", "flac");
    /** 豆包与 MiniMax 当前支持采样率的并集。 */
    private static final Set<Integer> TTS_SAMPLE_RATES = Set.of(8000, 16000, 22050, 24000, 32000, 44100, 48000);
    ...
    public AudioTaskVO generateAudioForParent(GenerateAudioRequest request, Long userId, Long parentTaskId) {
        ...
        task.setUpdateBy(String.valueOf(userId));
        task.setUpdateTime(DateUtils.getNowDate());
        // 只更新本方法拥有的 TTS 字段。同步媒体事件可能已在调用栈内写入 duration_ms、
        // sync_media_task_id / sync_video_url / enable_lip_sync，整实体 updateById 会把这些新状态覆盖回旧值。
        LambdaUpdateWrapper<AidAudioRecord> ttsUpdate = Wrappers.lambdaUpdate();
        ttsUpdate.eq(AidAudioRecord::getId, task.getId());
        ttsUpdate.set(AidAudioRecord::getTtsMediaTaskId, task.getTtsMediaTaskId());
        ttsUpdate.set(AidAudioRecord::getStatus, task.getStatus());
        ttsUpdate.set(AidAudioRecord::getErrorMessage, task.getErrorMessage());
        if (StrUtil.isNotBlank(task.getAudioUrl())) {
            ttsUpdate.set(AidAudioRecord::getAudioUrl, task.getAudioUrl());
        }
        ttsUpdate.set(AidAudioRecord::getUpdateBy, task.getUpdateBy());
        ttsUpdate.set(AidAudioRecord::getUpdateTime, task.getUpdateTime());
        aidAudioRecordService.update(ttsUpdate);

        // 同步厂商在 generateAudio 内部就会触发完成事件回填 duration_ms / audio_url，
        // 内存态 task 拿不到这些字段，回读一次以库内最终状态为准返回
        AidAudioRecord persisted = aidAudioRecordService.getById(task.getId());
        return buildAudioTaskVO(Objects.nonNull(persisted) ? persisted : task);
    }
    /**
    ...
    private ResolvedVoice resolveVoice(GenerateAudioRequest request) {
        ...
        if (Objects.nonNull(request.getVoiceLibraryId())) {
            ...
            if (StrUtil.isBlank(voice.getVoiceCode())) {
                log.info("generateAudio 音色编码为空, voiceLibraryId={}", request.getVoiceLibraryId());
                throw new ServiceException("音色不可用");
            }
            AidAiModel model = getAvailableAudioModel(voice.getModelId());
    ...
    /**
     * 配音公共参数前置校验。Controller 的 Bean Validation 只覆盖外部调用，批量配音和对口型等内部调用
     * 仍需在 Service 层校验，确保非法参数不会先创建业务记录。
     */
    private void validateCommonAudioParams(GenerateAudioRequest request) {
        if (Objects.nonNull(request.getEmotionScale())
                && (request.getEmotionScale() < 1 || request.getEmotionScale() > 5)) {
            log.info("generateAudio 情感强度非法: value={}", request.getEmotionScale());
        ...
        if (Objects.nonNull(request.getSpeechRate())
            ...
            log.info("generateAudio 语速非法: value={}", request.getSpeechRate());
        ...
        if (Objects.nonNull(request.getLoudnessRate())
            ...
            log.info("generateAudio 音量非法: value={}", request.getLoudnessRate());
            throw new ServiceException("音量无效");
        }
        if (Objects.nonNull(request.getPitch())
                && (request.getPitch() < -12 || request.getPitch() > 12)) {
            log.info("generateAudio 音调非法: value={}", request.getPitch());
        ...
        if (StrUtil.isNotBlank(request.getAudioFormat())) {
            ...
            if (!TTS_AUDIO_FORMATS.contains(format)) {
                log.info("generateAudio 音频格式非法: value={}", request.getAudioFormat());
                throw new ServiceException("音频格式无效");
            }
            request.setAudioFormat(format);
        }
        if (Objects.nonNull(request.getSampleRate()) && !TTS_SAMPLE_RATES.contains(request.getSampleRate())) {
            log.info("generateAudio 采样率非法: value={}", request.getSampleRate());
    ...
    public void setFinalSelection(SetFinalSelectionRequest request, Long userId) {
        ...
        if (Objects.equals(RECORD_TYPE_IMAGE, recordType) || Objects.equals(RECORD_TYPE_VIDEO, recordType)) {
            ...
            // 互斥范围：图片类整类互斥；分镜原视频与配音成片各自在同类中单选。
            // 原视频是配音成片的上游依赖，换选原视频时必须同步失效旧配音成片，避免后续导出继续使用旧源视频产物。
            ...
            boolean originalSourceChanged = Objects.equals(RECORD_TYPE_VIDEO, recordType)
                    && !composeTarget && !Objects.equals(storyboard.getFinalVideoId(), recordId);
            if (originalSourceChanged) {
                LambdaUpdateWrapper<AidGenRecord> invalidateComposeWrapper = Wrappers.lambdaUpdate();
                invalidateComposeWrapper.eq(AidGenRecord::getStoryboardId, request.getStoryboardId());
                invalidateComposeWrapper.eq(AidGenRecord::getDelFlag, DEL_FLAG_NORMAL);
                invalidateComposeWrapper.in(AidGenRecord::getGenType, COMPOSE_VIDEO_GEN_TYPES);
    ...
    public StoryboardVO getStoryboardDetail(StoryboardDetailRequest request, Long userId) {
        ...
        fillRecommendedDuration(storyboard, vo);
        // 配音展示字段与列表同口径
     ...
     * @param vo         分镜详情
     */
    private void fillRecommendedDuration(AidStoryboard storyboard, StoryboardVO vo) {
        String creationMode = resolveStoryboardCreationMode(storyboard);
        boolean useStoryboardSuggestion = Objects.equals(CreationModeEnum.MULTI.getValue(), creationMode)
                || Objects.equals(CreationModeEnum.PRO.getValue(), creationMode)
                || Objects.equals(CreationModeEnum.AUTO_GRID.getValue(), creationMode);
        Integer duration = useStoryboardSuggestion
                ? StoryboardDurationResolver.parseRecommendedDuration(storyboard.getScriptParams()) : null;
        if (Objects.nonNull(duration)) {
            vo.setRecommendedDurationSeconds(duration);
            vo.setRecommendedDurationSource(StoryboardDurationResolver.SOURCE_STORYBOARD_SUGGESTION);
            vo.setRecommendedDurationDescription("分镜建议" + duration + "秒，切换模型后按模型支持档位向上调整");
            return;
        }

        AiModelConfigVo defaultModel = resolveDefaultVideoModel(creationMode);
        duration = StoryboardDurationResolver.resolveModelDefaultDuration(defaultModel);
        vo.setRecommendedDurationSeconds(duration);
        vo.setRecommendedDurationSource(StoryboardDurationResolver.SOURCE_MODEL_DEFAULT);
        vo.setRecommendedDurationDescription(Objects.nonNull(duration)
                ? "模式默认" + duration + "秒，切换模型后按模型支持档位向上调整"
                : "当前模式未配置默认秒数");
    }
     ...
     * @return 创作模式
     */
    private String resolveStoryboardCreationMode(AidStoryboard storyboard) {
        if (Objects.nonNull(storyboard.getEpisodeId()) && storyboard.getEpisodeId() > 0) {
            // 防漏字段：创作模式只依赖剧集 ID 和 creation_mode，后续扩展取数请同步增列。
            AidComicEpisode episode = aidComicEpisodeService.getOne(
                    Wrappers.<AidComicEpisode>lambdaQuery()
                            .select(AidComicEpisode::getId, AidComicEpisode::getCreationMode)
                            .eq(AidComicEpisode::getId, storyboard.getEpisodeId())
                            .last("LIMIT 1"), false);
            if (Objects.nonNull(episode) && StrUtil.isNotBlank(episode.getCreationMode())) {
                return episode.getCreationMode().trim();
            }
        }
        if (Objects.nonNull(storyboard.getProjectId()) && storyboard.getProjectId() > 0) {
            // 防漏字段：创作模式只依赖项目 ID 和 default_creation_mode，后续扩展取数请同步增列。
            AidComicProject project = aidComicProjectService.getOne(
                    Wrappers.<AidComicProject>lambdaQuery()
                            .select(AidComicProject::getId, AidComicProject::getDefaultCreationMode)
                            .eq(AidComicProject::getId, storyboard.getProjectId())
                            .last("LIMIT 1"), false);
     ...
     * @return 默认视频模型配置；未配置时返回 null
     */
    private AiModelConfigVo resolveDefaultVideoModel(String creationMode) {
        String funcCode;
        if (Objects.equals(CreationModeEnum.PRO.getValue(), creationMode)) {
            funcCode = FUNC_CODE_STORYBOARD_VIDEO_MULTI_PRO;
        } else if (Objects.equals(CreationModeEnum.AUTO_GRID.getValue(), creationMode)) {
            funcCode = FUNC_CODE_STORYBOARD_VIDEO_GRID;
        } else if (Objects.equals(CreationModeEnum.I2V.getValue(), creationMode)) {
            funcCode = FUNC_CODE_STORYBOARD_VIDEO_IMAGE;
        } else {
            funcCode = FUNC_CODE_STORYBOARD_VIDEO;
        }
        try {
            List<AiModelVO> models = aiModelBusinessService.listAvailableModelsByFuncCode(funcCode);
            if (CollectionUtil.isEmpty(models) || StrUtil.isBlank(models.get(0).getModelCode())) {
                log.warn("分镜详情默认视频模型池为空: storyboardMode={}, funcCode={}", creationMode, funcCode);
                return null;
            }
            return aiModelConfigService.selectByModelCode(models.get(0).getModelCode());
        } catch (Exception e) {
            // 推荐信息是详情附加数据，模型池异常不应阻断分镜正文读取。
            log.warn("分镜详情默认视频时长解析失败: storyboardMode={}, funcCode={}", creationMode, funcCode, e);
            return null;
        }
    }
    ...
    public List<GenRecordVO> listGenRecordsByStoryboard(StoryboardGenRecordListRequest request, Long userId) {
        ...
        // storyboardId 为可选筛选条件，不传时保持项目 / 剧集维度的原查询范围。
        wrapper.eq(Objects.nonNull(request.getStoryboardId()),
                AidGenRecord::getStoryboardId, request.getStoryboardId());
    ...
    public GenRecordVO uploadStoryboardImage(UploadStoryboardImageRequest request, Long userId) {
        ...
        // 仅接受本站已上传资源的规范相对路径，拒绝站外外链、协议相对地址与路径穿越。
        ...
        for (SetFinalImageRequest.Item it : items) {
            ...
            try {
                ...
                log.error("批量最终产物-单条失败: sbId={}, recId={}, isSet={}, recordType={}, userId={}, err={}",
                        sbId, recId, isSet, recordType, userId, e.getMessage(), e);
                result.addFailure(recId,
                        TaskErrorPresentation.toUserMessage(e.getMessage(), "操作失败"));
            }
        }

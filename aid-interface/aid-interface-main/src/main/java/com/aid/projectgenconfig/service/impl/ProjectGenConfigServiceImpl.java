...
public class ProjectGenConfigServiceImpl implements IProjectGenConfigService
{
    ...
    public List<ProjectGenConfigVO> listConfig(Long projectId, Long episodeId, Long userId)
    {
        ...
        for (ProjectGenConfigScene scene : ProjectGenConfigScene.values())
        {
            ...
            // GET 接口同时治理历史脏值：切换模型后遗留的清晰度/比例按场景能力回退，
            // 保证自动生成拿到的配置与模型能力接口一致；直接生成接口仍保持严格校验。
    ...
    private void fillAgentOptions(List<ProjectGenConfigVO> result)
    {
        ...
        for (ProjectGenConfigVO vo : result)
        {
            ...
            // 当前选中智能体的图标直接随场景返回，调用方无需再遍历 agentOptions 匹配。
            AgentInfoVO selectedAgent = byCode.get(vo.getAgentCode());
            vo.setAgentIconUrl(Objects.isNull(selectedAgent) ? null : selectedAgent.getIconUrl());
    ...
    public int clearProjectConfig(Long projectId, Long userId)
    {
        ...
        // 物理删除全部配置并一并清理历史软删除残留，不再让旧记录占用唯一键
        int rows = projectGenConfigMapper.delete(
        ...
        log.info("物理清空项目级生成配置: projectId={}, userId={}, rows={}", projectId, userId, rows);
        return rows;
    }
    ...
    private AidProjectGenConfig validateItem(Long projectId, Long userId, ProjectGenConfigItem item)
    {
        ...
        boolean supportsAspectRatio = false;
                                            ...
                                            String resolution, String aspectRatio)
    {
        ...
        parseCapability(modelConfig.getCapabilityJson());
        List<String> sizeOptions = ModelCapabilityResolver.readImageSceneOptions(
                modelConfig, scene.getCapabilityScene(), ModelCapabilityResolver.KEY_SIZE_OPTIONS);
        List<String> aspectRatioOptions = ModelCapabilityResolver.readImageSceneOptions(
                modelConfig, scene.getCapabilityScene(), ModelCapabilityResolver.KEY_ASPECT_RATIO_OPTIONS);

        // 清晰度
        if (scene.isNeedResolution() && !optionHit(resolution, sizeOptions))
        {
        ...
        boolean supportsAspectRatio = Boolean.TRUE.equals(modelConfig.getSupportsAspectRatio());
        if (scene.isNeedAspectRatio() && supportsAspectRatio && StrUtil.isBlank(aspectRatio))
        {
            log.error("项目生成配置校验失败: 比例缺失, projectId={}, sceneCode={}, modelCode={}",
                    projectId, scene.getSceneCode(), modelCode);
            throw new ServiceException("参数缺失");
        }
        // 模型不支持独立比例参数时，比例由显式像素尺寸或输入图决定，不要求保存伪比例。
        if (scene.isNeedAspectRatio() && supportsAspectRatio
                && !optionHit(aspectRatio, aspectRatioOptions))
        {
    ...
    {
        return Objects.nonNull(ModelCapabilityResolver.matchOption(options, value));
    }

    /** 按场景能力修复 GET 返回中的历史清晰度/比例脏值。 */
    private void normalizeImageSceneConfig(ProjectGenConfigVO vo, ProjectGenConfigScene scene)
    {
        if (!scene.isImageScene() || Objects.isNull(vo) || StrUtil.isBlank(vo.getModelCode()))
        {
            return;
        }
        ...
        if (scene.isNeedResolution())
        {
            vo.setResolution(ModelCapabilityResolver.coerceImageSceneSize(
                    modelConfig, scene.getCapabilityScene(), vo.getResolution()));
        }
        if (scene.isNeedAspectRatio() && Boolean.TRUE.equals(modelConfig.getSupportsAspectRatio()))
        {
            vo.setAspectRatio(ModelCapabilityResolver.coerceImageSceneAspectRatio(
                    modelConfig, scene.getCapabilityScene(), vo.getAspectRatio()));
        }
        else
        {
    ...
    /** 已停用或移出场景模型池的存量配置，读取时回退矩阵默认或当前可用模型首项。 */
    private void repairUnavailableModel(ProjectGenConfigVO vo, GenAgentMatrixResult matrix,
                                        String mode, List<AiModelVO> availableModels)
    {
        if (Objects.isNull(vo))
        {
            return;
        }
        if (CollectionUtil.isEmpty(availableModels))
        {
            if (StrUtil.isNotBlank(vo.getModelCode()))
            {
                log.warn("项目配置场景无可用模型，GET 清除停用模型: sceneCode={}, oldModel={}",
                        vo.getSceneCode(), vo.getModelCode());
            }
            vo.setModelCode(null);
            vo.setResolution(null);
            vo.setAspectRatio(null);
        ...
        String matrixModelCode = Objects.isNull(matrix) ? null : StrUtil.trim(matrix.getModelCode());
        String fallbackModelCode = availableModels.stream()
                .map(AiModelVO::getModelCode)
                .filter(code -> Objects.equals(code, matrixModelCode))
                .findFirst()
                .orElse(availableModels.get(0).getModelCode());
        log.warn("项目配置模型已停用或移出模型池，GET 回退可用模型: sceneCode={}, oldModel={}, newModel={}",
                vo.getSceneCode(), vo.getModelCode(), fallbackModelCode);
        vo.setModelCode(fallbackModelCode);
        vo.setResolution(null);
        vo.setAspectRatio(null);
        vo.setSource(SOURCE_DEFAULT);
        vo.setMode(mode);
    }
    /**
     * 按项目+用户+场景唯一键原子保存：首次新增，后续更新。
     */
    private void upsert(AidProjectGenConfig entity, String operator)
    {
        // 新增审计字段仅在首次插入时落库，唯一键更新时由 SQL 保留原创建信息
        entity.setCreateBy(operator);
        entity.setCreateTime(DateUtils.getNowDate());
        entity.setUpdateBy(operator);
        entity.setUpdateTime(DateUtils.getNowDate());
        entity.setDelFlag(DEL_FLAG_NORMAL);
        projectGenConfigMapper.upsertByUniqueKey(entity);
    }
    /** 无任何配置时的占位 VO。 */

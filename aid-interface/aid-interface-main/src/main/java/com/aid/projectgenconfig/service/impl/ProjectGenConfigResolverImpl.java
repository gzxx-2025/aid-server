...
public class ProjectGenConfigResolverImpl implements IProjectGenConfigResolver
{
                                       ...
                                       String requestedResolution, String requestedAspectRatio)
    {
        ...
        boolean modelExplicit = StrUtil.isNotBlank(modelCode);
        boolean resolutionExplicit = StrUtil.isNotBlank(resolution);
        boolean aspectRatioExplicit = StrUtil.isNotBlank(aspectRatio);
        ...
        if (!inPool)
        {
            if (modelExplicit)
            {
                log.error("项目配置解析失败: 显式模型不在可选池内, projectId={}, sceneCode={}, modelCode={}, poolSize={}",
                        projectId, sceneCode, modelCode, pool.size());
                throw new ServiceException("模型不支持");
            }
            String fallbackModelCode = resolveAvailableModelCode(matrix, pool);
            log.warn("项目存量模型已停用或移出模型池，回退可用模型: projectId={}, sceneCode={}, oldModel={}, newModel={}",
                    projectId, sceneCode, modelCode, fallbackModelCode);
            modelCode = fallbackModelCode;
            // 模型变化后历史图片参数不再可信；仅保留调用方本次显式传入的参数。
        ...
        {
            // 保持 capability_json 必须可解析的既有约束，同时按实际场景读取更窄的白名单。
            ...
            if (!resolutionExplicit)
            {
                resolution = ModelCapabilityResolver.coerceImageSceneSize(
                        modelConfig, scene.getCapabilityScene(), resolution);
            }
            if (!supportsAspectRatio)
            {
                // 比例由显式像素尺寸或输入图决定，禁止把历史伪比例继续传入媒体主链路。
                aspectRatio = null;
            }
            else if (!aspectRatioExplicit)
            {
                aspectRatio = ModelCapabilityResolver.coerceImageSceneAspectRatio(
                        modelConfig, scene.getCapabilityScene(), aspectRatio);
            }
            if (scene.isNeedResolution() && StrUtil.isBlank(resolution))
            {
            ...
            if (scene.isNeedResolution() && !optionHit(resolution, sizeOptions))
            {
                log.error("项目配置解析失败: 清晰度不在模型能力内, projectId={}, sceneCode={}, modelCode={}, resolution={}, sizeOptions={}",
                        projectId, sceneCode, modelCode, resolution, sizeOptions);
                    ...
                    && !optionHit(aspectRatio, aspectRatioOptions))
            {
                log.error("项目配置解析失败: 比例不在模型能力内, projectId={}, sceneCode={}, modelCode={}, aspectRatio={}, aspectRatioOptions={}",
                        projectId, sceneCode, modelCode, aspectRatio, aspectRatioOptions);
    ...
    /** 优先回退矩阵默认模型；矩阵模型不可用时取模型池首项。 */
    private String resolveAvailableModelCode(GenAgentMatrixResult matrix, List<AiModelVO> pool)
    {
        String matrixModelCode = Objects.isNull(matrix) ? null : StrUtil.trim(matrix.getModelCode());
        if (StrUtil.isNotBlank(matrixModelCode)
                && pool.stream().anyMatch(model -> Objects.equals(matrixModelCode, model.getModelCode())))
        {
            return matrixModelCode;
        }
    ...
    /** 候选项命中：复用 {@link ModelCapabilityResolver} 的归一化口径，保证与媒体主链路校验同源。 */

 ...
 * <p>支持的台词形态（可混合出现）：</p>
 * <ul>
 *   <li>视觉导演台词行：{@code 台词：【罗峰_初始形象，低沉独白】@音频1[音频-罗峰]说："…"，语速缓慢深沉。}</li>
 *   <li>分镜台词列简格式：{@code [罗峰_初始形象]："…" | [巴巴塔_初始形象]："…"}</li>
 *   <li>富文本编辑器写入的 HTML 段落（{@code <p>…</p>}）→ 归一为纯文本后按换行分段</li>
...
public class StoryboardAudioReferenceResolver
{
    ...
    public List<DialogueSegment> parse(String dialogueText)
    {
        ...
        // 富文本归一：编辑器写入的 <p>/<br> 转为换行参与分段，段首角色标记才能落在段首被识别
        String plainText = RichTextNormalizer.toPlainText(dialogueText);
        if (StrUtil.isBlank(plainText))
        {
            return segments;
        }
        // 分段：竖线 / 换行统一切开，空段丢弃
        String[] rawSegments = SEGMENT_SEPARATOR.split(plainText);
        int index = 0;
    ...
    private DialogueSegment parseSegment(String rawSegment)
    {
        ...
        // 3) @音频N[...] 占位：提取编号后删除（正则统一收口在 StoryboardAudioPlaceholders）
        if (StoryboardAudioPlaceholders.contains(rest))
        {
            segment.setAudioRefN(StoryboardAudioPlaceholders.firstIndex(rest));
            rest = StoryboardAudioPlaceholders.removeAll(rest);
        }

...
public interface IAssetExtractService
{
    ...
    /**
     * 按派发周期执行资产提取，防止失活 worker 在新一轮续跑后继续写入。
     */
    List<RpsAssetVO> doExtract(Long taskId, Long projectId, Long episodeId, Long userId,
     ...
     *
     * @param taskId        任务ID
     * @param dispatchToken 派发周期令牌
     */
    void releaseTaskSlots(Long taskId, String dispatchToken);
    ...
    /**
     * 原子领取 MQ 任务：仅当前 PENDING 且派发周期匹配时推进到 PROCESSING。
     * 无令牌消息一律拒绝，避免旧消息领取新的执行周期。
     */
    boolean claimTaskForExecution(Long taskId, String dispatchToken);

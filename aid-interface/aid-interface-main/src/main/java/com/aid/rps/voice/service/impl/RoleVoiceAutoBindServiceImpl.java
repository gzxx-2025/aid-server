...
public class RoleVoiceAutoBindServiceImpl implements IRoleVoiceAutoBindService
{
    ...
    public void upsertBinding(AidRolePropScene role, AidAiVoiceLibrary voice, Long userId)
    {
        ...
        else if (Objects.equals(previousSampleUrl, entity.getSampleUrl()))
        {
            bindingService.updateById(entity);
        }
        else
        {
            // 试听时长是 sample_url 的冗余，换音色后旧值失效，必须在同一条 UPDATE 里显式置空
            bindingService.update(entity, Wrappers.<AidRoleVoiceBinding>lambdaUpdate()
                    .eq(AidRoleVoiceBinding::getId, entity.getId())
                    .set(AidRoleVoiceBinding::getSampleDurationMs, null));
        }
    }
}

...
public class RpsBusinessServiceImpl implements IRpsBusinessService {
    ...
    public BatchOperationResultVO deleteAssetBatch(List<Long> ids, Long userId)
    {
        ...
        for (Long assetId : ids)
        {
            ...
            {
                // 单条失败：记录原因并继续处理后续条目
                log.error("批量删除资产-单条失败: assetId={}, userId={}, err={}",
                        assetId, userId, e.getMessage(), e);
                result.addFailure(assetId,
    ...
    public BatchOperationResultVO useFormBatch(Long projectId, List<Long> imageIds, Long userId)
    {
        ...
        for (Long imageId : imageIds)
        {
            ...
            {
                // 单条失败：记录原因并继续处理后续条目
                log.error("批量设置使用中-单条失败: imageId={}, userId={}, err={}",
                        imageId, userId, e.getMessage(), e);
                result.addFailure(imageId,
                        TaskErrorPresentation.toUserMessage(e.getMessage(), "操作失败"));
            }
        }
    ...
    public BatchOperationResultVO unuseFormBatch(Long projectId, List<Long> imageIds, Long userId)
    {
        ...
        for (Long imageId : imageIds)
        {
            ...
            {
                log.error("批量取消使用中-单条失败: imageId={}, userId={}, err={}",
                        imageId, userId, e.getMessage(), e);
                result.addFailure(imageId,
                        TaskErrorPresentation.toUserMessage(e.getMessage(), "操作失败"));
            }
    ...
    private com.fasterxml.jackson.databind.node.ObjectNode parseStoredProfileDataLenient(String assetType, Long assetId, String storedProfileData)
    {
        ...
        try
        {
            ...
            if (Objects.isNull(parsed) || !parsed.isObject())
            {
                log.warn("库存 profileData 非 JSON 对象，降级为空对象继续编辑: assetId={}, assetType={}, contentLen={}",
                        assetId, assetType, StrUtil.length(storedProfileData));
                return OBJECT_MAPPER.createObjectNode();
            }
            return (com.fasterxml.jackson.databind.node.ObjectNode) parsed;
        }
    ...
    private void setAvailableSlotsNode(com.fasterxml.jackson.databind.node.ObjectNode node, String availableSlotsJson)
    {
        try
        {
            ...
            {
                log.info("availableSlots 非 JSON 数组, contentLen={}", StrUtil.length(availableSlotsJson));
    ...
    private void validateAvailableSlotsJson(String availableSlots)
    {
        ...
        try
        {
            ...
            {
                log.info("availableSlots 非 JSON 数组, contentLen={}", StrUtil.length(availableSlots));
                throw new RuntimeException("槽位格式错");
            }
        }
        catch (RuntimeException e)

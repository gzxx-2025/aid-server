        ...
        try {
            ...
            if (taskResult == null
                ...
                if (taskResult != null
                        ...
                        && MediaTaskStatus.PROCESSING.name().equals(taskResult.getStatus())) {
                    // 反查 MPS 拿到非终态，即上游仍在推进的实证：登记进展，避免长片合成被无进展时钟误判超时。
                    // taskResult 为空说明反查失败，此时上游生死未知，不得登记，否则失联任务永远判不了死。

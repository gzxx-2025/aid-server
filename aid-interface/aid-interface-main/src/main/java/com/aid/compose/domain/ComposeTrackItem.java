...
public class ComposeTrackItem {
    ...
    /**
     * 视频在轨道上的占用时长（秒），仅裁剪播放的视频项非空。
     * 与 sourceStartSeconds/sourceEndSeconds 取用区间等长，即原速播放。
     */
    private Double trackDurationSeconds;

    /** 素材取用起点（秒），与 sourceEndSeconds 成对出现；为空表示整条素材原样播放 */
    private Double sourceStartSeconds;

    /** 素材取用终点（秒），与 sourceStartSeconds 成对出现；为空表示整条素材原样播放 */
    private Double sourceEndSeconds;
}

...
public class MediaTask {
    ...
    /**
     * 统一调度中心 - 自定义批次大小
     * 包含：到期轮询、回调转轮询、存活异常对账、未提交僵尸收口与排队拉起。
     *
    ...
    public void dispatch(Integer batchSize) {
        ...
        try {
            ...
            // 3) 存活异常对账：只查询上游权威状态，不按本地超时关闭已有上游任务ID的任务。
            int reconciled = taskDispatchService.closeTimeoutTasks(limit);

            // 4) 提交阶段僵尸收口：仅 PENDING 且无 providerTaskId 且超兜底时限；QUEUED 合法排队不清理。
            int staleClosed = taskDispatchService.closeStaleUnsubmittedTasks(limit);

            // 5) 排队任务兜底拉起：完成事件丢失 / 重启后仍按 FIFO 扫描 QUEUED，尝试抢占并发坑位拉起（CAS 防重）。
            int queuedDrained = mediaGenerationService.drainQueuedCompensate(limit);

            if (polled > 0 || expired > 0 || reconciled > 0 || staleClosed > 0 || queuedDrained > 0) {
                log.info("调度中心执行完成, polled={}, expired={}, reconciled={}, staleClosed={}, queuedDrained={}",
                    polled, expired, reconciled, staleClosed, queuedDrained);
            }
        } finally {

...
public final class FixedJobRegistry
{
    ...
    private static final List<JobDef> DEFS = List.of(
            ...
            fixed("mediaTask.dispatch", "mediaTask.dispatch", "媒体任务统一调度中心",
                    "0/10 * * * * ?", 10, 5, 30, ScheduleConstants.MISFIRE_DO_NOTHING,
                    "异步图片/视频/音频任务推进核心：到期轮询、回调超时转轮询、生命周期对账、提交僵尸收口、排队兜底拉起。停用后任务无法到达终态"),
            ...
            optional("assetCleanupTask.cleanExpired", "assetCleanupTask.cleanExpired()", "自动覆盖资产墓碑清理",
                    "0 20 3 * * ?", DAILY_INTERVAL_SECONDS, ScheduleConstants.Status.NORMAL.getValue(),
                    "每日清理达到保留期的自动覆盖资产墓碑及其不再被引用的对象存储文件"),

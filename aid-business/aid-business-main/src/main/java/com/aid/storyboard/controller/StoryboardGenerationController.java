...
public class StoryboardGenerationController extends BaseController {
     ...
     * 返回父任务 taskId，前端订阅任务 SSE 获取进度：配音阶段的 progress 事件带 audioRecordId /
     * audioUrl / durationMs，终态 complete 事件的 items[0] 带对口型视频记录ID与地址。
     * 同分镜已有在跑的对口型任务时幂等返回该任务 taskId，供前端直接重连 SSE。
     * 产物落配音视频记录（genType=compose，不自动选中，用户可手动设为配音视频主视频）。
     */
    @PostMapping("/lipSync")
    public AjaxResult lipSync(@Valid @RequestBody LipSyncRequest request) {
        Long userId = SecurityUtils.getUserId();
        AssetExtractTaskVO vo = storyboardLipSyncService.lipSync(request, userId);
        wechatNotifyService.notifyTaskStarted(vo.getTaskId());
        return success(vo);
    }
    ...
    /**
     * 解析分镜提示词中的图片、角色音色与用户音频引用。
     *
     * @param request 待解析的 image_prompt + 项目/剧集归属
     * @return 图片与参考音频的结构化解析结果
     */
    ...
    public AjaxResult resolveImagePromptReferences(
        ...
        Long userId = SecurityUtils.getUserId();
        StoryboardImageReferenceResolver.MediaResolveResult vo =
                storyboardImageReferenceResolver.resolveMedia(
                        request.getImagePrompt(),
                        request.getProjectId(),
                        request.getEpisodeId(),
                        userId,
                        request.getReferenceAudioRecordIds(),
                        request.getReferenceAudioIds());
        return success(vo);
    }

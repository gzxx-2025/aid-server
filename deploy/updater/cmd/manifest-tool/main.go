...
func verifyManifest(keyPath, manifestPath string) error {
	...
	if payloadText, ok := signatureObject["payload"].(string); ok && strings.TrimSpace(payloadText) != "" {
		signedPayload, err = base64.StdEncoding.DecodeString(payloadText)
		if err != nil {
			return fmt.Errorf("invalid manifest signature payload: %w", err)
		}
		var payloadDocument map[string]any
		if err := json.Unmarshal(signedPayload, &payloadDocument); err != nil {
			return fmt.Errorf("invalid manifest signature payload: %w", err)
		}
		payloadCanonical, err := json.Marshal(payloadDocument)
		if err != nil || string(payloadCanonical) != string(canonical) {
			return fmt.Errorf("manifest signature payload does not match manifest")
		}
	}
	if !ed25519.Verify(privateKey.Public().(ed25519.PublicKey), signedPayload, signature) {
